...
└<b>WhatsApp</b>: {utl2}"""
    ...
    def send_text(message):
        ...
        elif message.text == '🔎 user-agent':
            bot.register_next_step_handler(msg, gen_agent)
...
''', 
        ...
        if call.data == 'gen_agent':
            try:
                ua = UserAgent(os_family = 'ios')
                res = ua.random()
                bot.edit_message_text(chat_id=u, message_id=message_id, text=f'''✅ user-agent успешно Перехвачен:\n\n🔴 {res} \n📌 получен!''', reply_markup=kb.gen_agent)
            except Exception as e:
                print(e)
                bot.edit_message_text(chat_id=u, message_id=message_id, text=f'''Ошибка''', reply_markup=kb.gen_agent, parse_mode='HTML')
